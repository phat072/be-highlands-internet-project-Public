...
async function main() {
  ...
  const admin = await prisma.user.upsert({ where: { email: 'admin@viettel.local' }, update: {}, create: { name: 'Quản trị dự án', email: 'admin@viettel.local', passwordHash: await argon2.hash('Admin@123'), roleId: adminRole.id, mustChangePassword: true } });
  ...
  if (await prisma.site.count()) return;
  const sites = [
